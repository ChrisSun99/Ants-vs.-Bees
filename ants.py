...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        return self.place.ant != None and self.place.ant.blocks_path is True
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place != hive and place.entrance != None:
            if len(place.bees)> 0 and self.min_range <= move <= self.max_range:
    ...
    def action(self, colony):
        ...
        if self.impostor == True:
            ...
            while forward is not None:
                #ant_to_buff = forward.ant

                if forward.ant is not None and not isinstance(forward.ant, QueenAnt):
                    if (isinstance(forward.ant, BodyguardAnt) or isinstance(forward.ant, TankAnt)) and forward.ant.ant and forward.ant.ant not in self.ants_behind:
                        forward.ant.ant.damage *= 2
                        QueenAnt.ants_behind.append(forward.ant.ant)
                    if forward.ant not in self.ants_behind:
                        forward.ant.damage *= 2
                        QueenAnt.ants_behind.append(forward.ant)
